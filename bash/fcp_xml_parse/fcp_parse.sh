...
projectfile="GENEVA_FINAL-071311-FOR_DVD.xml"
...
if [ ${start} -eq -1 ]; then
    ...
    precedingTransition=$(grep -B 18 "<clipitem id=\"${imagename}\">" "${projectfile}" | tr -d "\n\t " )
    start=$(echo "${precedingTransition}" | sed 's/.*\<start\>\(.*\)\<\/start\>.*/\1/')
    echo "${imagename} ${start}" >> geneva_images_with_start_transitions.txt
...
if [ ${end} -eq -1 ]; then
	...
	echo "${imagename} ${end}" >> geneva_images_with_end_transitions.txt
...
echo -e  "${imagename}\t${startminutes}:${startseconds}:${startframes}\t${endminutes}:${endseconds}:${endframes}\t${durminutes}:${durseconds}:${durframes}" >> imagesummary.txt
fi
done
echo "" > image_columns.txt
column -t imagesummary.txt >> image_columns.txt
